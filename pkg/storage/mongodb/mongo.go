...
func (db *MongoDb) GetValue(database, collection string, query bson.M, result interface{}) error {
	...
	if err != nil {
		return errors.E(err, errors.Params{"database": database, "collection": collection, "query": query})
	}
	return nil
}
...
func (db *MongoDb) InsertMany(database, collection string, value []interface{}) ([]interface{}, error) {
	...
	if err != nil {
		return nil, errors.E(err, errors.Params{"database": database, "collection": collection, "query": value})
	}
...
func (db *MongoDb) DeleteOne(database, collection string, query bson.M) (int64, error) {
	c := db.Database(database).Collection(collection)
	res, err := c.DeleteOne(context.TODO(), query)
	if err != nil {
		return 0, errors.E(err, errors.Params{"database": database, "collection": collection, "query": query})
	}
